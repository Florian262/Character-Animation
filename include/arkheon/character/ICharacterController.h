...
typedef struct {
    arkheon_quat local_rotation;     /* parent-relative quaternion (16 bytes) */
    arkheon_vec3 local_translation;  /* parent-relative position (12 bytes) */
    float        _pad0;              /* alignment padding (4 bytes) */
    arkheon_vec3 world_position;     /* model-space (12 bytes) */
    float        _pad1;              /* alignment padding (4 bytes) */
} arkheon_bone_state;                /* Total: 48 bytes */
...
typedef struct {
    arkheon_quat local_rotation;  /* physics output (16 bytes) */
    uint8_t      apply;           /* override flag (1 byte) */
    uint8_t      _pad[15];        /* padding to 32-byte boundary (15 bytes) */
} arkheon_bone_override;          /* Total: 32 bytes */
...
typedef struct {
    ...
    uint8_t  _pad[12];         /* alignment padding */
} arkheon_input_state;
...
#endif // I_CHARACTER_CONTROLLER_H
